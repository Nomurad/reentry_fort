module mod_vector
    ...
    type, public :: t_Vector
        ...
        ! real(real64), allocatable, public :: vec(:)
    ...
    interface t_Vector 
        module procedure init_vec1, init_vec2, init_vec3 
    end interface

    contains
        type(t_Vector) pure function init_vec1(x, y, z) result(res)
        ...
        type(t_Vector) pure function init_vec2(ndim) result(res)
            integer, intent(in) :: ndim

            res%ndim = ndim
            allocate(res%vec(ndim), source=0d0)

        end function init_vec2

        type(t_Vector) pure function init_vec3(vector) result(res)
            class(t_Vector), intent(in) :: vector 
            
            res%ndim = vector%ndim 
            allocate(res%vec(vector%ndim))
            res%vec = vector%vec

        end function init_vec3
        ...
        subroutine formatted_write(this,unit,iotype,vlist,iostat,iomsg)
            ...
            ! write(unit, "(3f20.14)", iostat=iostat) this%vec
            write(unit, *, iostat=iostat) this%vec
        end subroutine
